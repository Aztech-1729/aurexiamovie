...
def strict_title_match(title: str, query: str) -> bool:
    ...
    # For single-word queries (like "Godzilla" or "Dhurandhar")
    if len(query_words) == 1:
        main_word = query_words[0]
        # Check if title contains the word anywhere (not just at start)
        if main_word in title_clean:
            return True
        # Also check with removed spaces
        if main_word in title_clean.replace(" ", ""):
            return True
        return False
