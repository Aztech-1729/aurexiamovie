...
# Suppress the ddgs user agent warning
warnings.filterwarnings("ignore", message=".*firefox_109.*")
...
from config import (
    ...
    WEB_SERVER_PORT, STREAM_SITE_DOMAIN
)
from filters import apply_filters, clean_title

# ── Dynamic Configuration ─────────────────────────────────────────────────────
PLAYER_BASE_URL = "http://localhost:5000"  # Will be updated by Cloudflare Tunnel
...
@flask_app.route('/')
def index():
    return "🎬 Aurexia Movie Bot - Server Running!<br>Use /play?id=tt1234567 to play movies."

@flask_app.route('/play')
def play_video():
    movie_id = request.args.get('id')
    item_type = request.args.get('type', 'movie')  # Default to movie
    title = request.args.get('title', 'Movie Player')
    if not movie_id:
        return render_template('index.html', embed_url=None)

    # StreamIMDb accepts full IMDb ID (e.g., tt5950044)
    embed_url = f"https://{STREAM_SITE_DOMAIN}/embed/{item_type}/{movie_id}"
    return render_template('index.html', embed_url=embed_url, title=title)

def run_flask():
    print(f"🌐 Starting web player on port {WEB_SERVER_PORT}...")
...
def do_search(query: str) -> list:
    ...
    from filters import apply_filters as apply_imdb_filters
    from filters import search_tv_series_seasons

    # First, try main search
    for attempt in range(1, 4):
        try:
            ...
            raw_results = ddgs.text(
                f"site:imdb.com {query}",
                region="in-en",
                safesearch="off",
                max_results=25,
            )

            filtered = apply_imdb_filters(raw_results, query)

            if filtered:
                # Check if it looks like a TV series
                has_tv = any(r.get("type") == "tv" for r in filtered)
                is_series_query = any(kw in query.lower() for kw in ["show", "series", "season", "tv"])

                if has_tv and is_series_query:
                    print(f"[Search] Detected TV series, searching for seasons too...")

                    # Search for seasons
                    season_queries = search_tv_series_seasons(query)
                    season_results = []

                    for sq in season_queries[:3]:  # Search first 3 seasons
                        try:
                            season_raw = ddgs.text(
                                f"site:imdb.com {sq}",
                                region="in-en",
                                safesearch="off",
                                max_results=10,
                            )
                            season_filtered = apply_imdb_filters(season_raw, sq)
                            season_results.extend(season_filtered)
                        except:
                            pass

                    # Add season results (avoid duplicates)
                    existing_ids = {r["movie_id"] for r in filtered}
                    for sr in season_results:
                        if sr["movie_id"] not in existing_ids:
                            sr["position"] = len(filtered) + 1
                            filtered.append(sr)
                            existing_ids.add(sr["movie_id"])

                cache_set(query, filtered)
                print(f"[Search] Found {len(filtered)} results (including seasons)")
                return filtered
            else:
                # Try alternative search
                print(f"[Search] No valid results on attempt {attempt}, trying alternative...")
                alt_results = ddgs.text(
                    f'{query} site:imdb.com/title',
                    region="in-en",
                    safesearch="off",
                    max_results=15,
                )
                filtered = apply_imdb_filters(alt_results, query)
                if filtered:
                    cache_set(query, filtered)
                    print(f"[Search] Found {len(filtered)} results with alternative search")
                    return filtered
...
def build_results_keyboard(results: list, page: int) -> InlineKeyboardMarkup:
    """Build keyboard with result buttons that point to the local player"""
    keyboard = []
    ...
    for i in range(start_idx, end_idx):
        ...
        item_type = result.get("type", "movie")

        # Construct player link - now uses IMDb ID (e.g., tt5950044)
        encoded_title = urllib.parse.quote(title)
        player_link = f"{PLAYER_BASE_URL}/play?id={movie_id}&type={item_type}&title={encoded_title}"

        # Truncate title if too long
        if len(title) > 40:
            title = title[:37] + "..."
        keyboard.append([InlineKeyboardButton(f"🎬 {i+1}. {title}", url=player_link)])
...
async def main_async():
    ...
    global PLAYER_BASE_URL
    print(f"☁️ Starting Cloudflare Tunnel (auto-downloading binary if needed)...")
    
    # Use with-cloudflared to handle everything automatically
    with cloudflared(port=WEB_SERVER_PORT) as public_url:
        PLAYER_BASE_URL = public_url
        print(f"✅ Cloudflare Tunnel Active: {PLAYER_BASE_URL}")
        
        await asyncio.gather(
            run_file_manager_bot_async(),
            run_main_bot_async(),
        )
        print("✅ Both bots and web server started! Waiting...", flush=True)
...
def main():
    print("🚀 Starting bots and web player...", flush=True)
